...
const float termistor[43][3] = { //  43 ??? ???????? {???????????, ?????????????, a}         ????????? epcos 8016 ?????, ????????????? 10 ???????     (????????? ???????? ?? datasheet)
...
float NTC_read(unsigned char polozenie_termistora)
{
  //        TOP viev   on KLEMNIK
  //   * * * *           * * * * * * * *
  // TRANSISTORS         8 7 6 5 4 3 2 1

  uint32_t all_summ = 0;
  float result = 0;

  if (polozenie_termistora == 1)
  {
    int i = 2;
    while (i <= 3000)
    {
      all_summ += SDADC1data3[i];
      i = i + 3;
    }
  }

  if (polozenie_termistora == 2)
  {
    int i = 1;
    while (i <= 3000)
    {
  ...
  if (polozenie_termistora == 4)
  {
    int i = 1;
    while (i <= 2000)
    {
      all_summ += SDADC2data2[i];
      i = i + 2;
    }
  }

  if (polozenie_termistora == 5)
  {
    int i = 0;
    while (i <= 2000)
    {
  ...
  if (polozenie_termistora == 6)
  {
    int i = 2;
    while (i <= 3000)
    {
      all_summ += SDADC3data3[i];
      i = i + 3;
    }
  }

  if (polozenie_termistora == 7)
  {
    int i = 1;
    while (i <= 3000)
    {
  ...
  result = all_summ / 1000;
  float resistance_ntc = ((65534 - result) / (result)) * 1.0 + 0.0; //Calculate resistence NTC (if 3.3 65534, if 3.0 59577
  ...
  result = (1.00 / (((((log(Rt / RTx)) / (a / 100.00 * (Tx + 273.15) * (Tx + 273.15)))) + 1.00 / (Tx + 273.15)))) - 273.15;
  return result;
}
...
char PidBigBlock(float SetTemp)
{
  ...
  static int first_call = 'y'; // static - ?????????? ?????????? (???? ???????? ?? n ?? ??????? ?? ?????????? ??? ????????? ?????? ?-??) + ????? ?????? ? ??? ?????? ???
  ...
  static float vozdeistvie, Integralnoe_proshloe, Kp = 29, Ki = 5, Kd = 9, Oshibka, proshlOhibka; // vozdeystvie_proshloe // ?????????? ? ????????? ??? ?????????? ??? Kp Ki ? Kd
  if (first_call == 'y')
  {
    first_call = 'n';
